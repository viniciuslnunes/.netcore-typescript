...
const Home: React.FC = () => {
  ...
  const loadBooks = useCallback(() => {
    axios
      .get<IBook[]>(`${apiConfig.baseUrl}/livros`, {})
  ...
  const handleDelete = useCallback((bookId: number) => {
    if (confirm('Vai jogar fora memo, irmão?')) {
      axios
        .delete<IBook[]>(`${apiConfig.baseUrl}/livros/${bookId}`, {})
        .then(() => loadBooks())
        .catch((err) => console.log(err))
    }
  }, [])
  ...
  const handleEdit = useCallback((bookId: number) => {
    history.push(`${routesMap.Settings.path}?bookId=${bookId}`)
  }, [])
    ...
    <SC.Container>
      ...
      <SC.MainContent>
        {books.length ? (
          ...
          <strong>Tem livro não, maluko. Sai daqui</strong>
        )}
      </SC.MainContent>
    </SC.Container>
  )
